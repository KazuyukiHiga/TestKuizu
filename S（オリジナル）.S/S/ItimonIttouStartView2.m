...
@implementation ItimonIttouStartView2
...
-(id)init
{
    self = [super initWithNibName:@"ItimonIttouStartView2" bundle:nil];
...
- (void)viewDidLoad
{
    ...
    [self imageview];
    [self showNextQuiz];
    NSString *toi1 = [NSString stringWithFormat:@"問%d",num22];
    mylabel1.text = toi1;
    seikaii = 0;
    num22 = 1;
    id ob = [[Quiz alloc] init];
...
-(void)imageview
{   
    id obbb = [[ItimonIttou2 alloc]init];
    
    nummm = [obbb gazouzousin];
    
    
    switch (nummm) {
        case 1:
            myimageview.image = [UIImage imageNamed:@"_C026577_R.jpg"];
            break;
        case 2:
        ...
        default: myimageview.image = [UIImage imageNamed:@"1400-800.jpg"];
            break;
    }
...
-(void)nextQuiz:(id)senfer
{
    //出題終えたら解答画面に移動
    if ([self.quiz.usedQuizItems count] == kQuizCount) {
        kaitouView2 *controller = 
        [[kaitouView2 alloc]
         initWithNibName:@"kaitouView2" bundle:nil];
